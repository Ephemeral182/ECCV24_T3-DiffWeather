...
def parse_args_and_config():
    ...
    parser.add_argument("--config", type=str,
                        default=str(config_path.joinpath("allweather_raindrop.yml")),
                        help="Path to the config file")
    parser.add_argument("--model_path", type=str, default="pretrained/T3_DiffWeather.ckpt",
                        help="Path to model")
    ...
    parser.add_argument("--image_folder_test", default='./save_images_test_', type=str,
...
class DiffusionSampler(torch.nn.Module):
    ...
    def sample_test(self):
        ...
        for i, batch in tqdm(enumerate(self.data_loader), total=len(self.data_loader), desc="Testing"):
            ...
            psnr = batch_PSNR(output.float(), gt.float(), ycbcr=True)
            ssim = batch_SSIM(output.float(), gt.float(), ycbcr=True)
        ...
        print('avg_psnr', average_psnr)
        print('avg_ssim', average_ssim)

if __name__ == "__main__":
    args, config = parse_args_and_config()
    
    if config.data.dataset == 'Test1':
        val_set = Test1(config.data.val_data_dir,train=False,crop=False)
    if config.data.dataset == 'Raindrop':
        val_set = AGAN_Dataset(config.data.val_data_dir,train=False,crop=False)
    if config.data.dataset == 'Snow100k-S':
        val_set = Snow100kTest(config.data.val_data_dir,train=False,crop=False)
    if config.data.dataset == 'Snow100k-L':
        val_set = Snow100kTest(config.data.val_data_dir,train=False,crop=False)
